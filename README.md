...
- `:fizz`: Elixir and Zig hybrid, ship the libffi functionality in Elixir. This app also handle the C library opening and provides functions and types to work with C data structures.
...
SomeDialect.some_op(
  regions: fn -> do
    region = Beaver.Env.mlir__REGION__() # first region created
    block = Beaver.Env.mlir__BLOCK__()
    x = Arith.constant(...)
    y = Arith.constant(...)

    region = Beaver.Env.mlir__REGION__() # second region created
    block = Beaver.Env.mlir__BLOCK__()
