...
fn Invocation(
    ...
    return struct {
        ...
        packed_args: [ArgCount + 2]?*anyopaque = undefined, // [arg0, arg1... result]
        fn init(self: *@This(), environment: beam.env, args: *const [2]c_ulong) !void {
            self.packed_args[0] = @ptrCast(@constCast(&environment));
            inline for (0..ArgCount) |idx| {
                self.packed_args[idx + 1] = @ptrCast(@constCast(&args[idx]));
            }
            self.packed_args[ArgCount + 1] = &self.res_term;
        }
...
fn beaver_raw_jit_invoke_with_terms(env: beam.env, _: c_int, args: [*c]const beam.term) !beam.term {
    ...
    var invocation = Invocation(2){};
    try invocation.init(env, args[2..4]);
...
const ArgSize = 2;
pub const nifs = .{
    result.nif("beaver_raw_jit_invoke_with_terms", 1 + ArgSize + 1, beaver_raw_jit_invoke_with_terms).entry,
