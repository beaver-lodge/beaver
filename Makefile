...
cmake_build:
	cmake -G Ninja -S native/mlir-c -B ${CMAKE_BUILD_DIR} -DLLVM_DIR=${LLVM_CMAKE_DIR} -DMLIR_DIR=${MLIR_CMAKE_DIR} -DCMAKE_INSTALL_PREFIX=${NATIVE_INSTALL_DIR} -DCMAKE_CXX_FLAGS="-static-libstdc++" -DCMAKE_SHARED_LINKER_FLAGS="-stdlib=libc++ -lc++abi"
