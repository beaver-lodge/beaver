...
zig_build:
	zig translate-c ${BEAVER_INCLUDE_DIR}/mlir-c/Beaver/wrapper.h --cache-dir ${ZIG_CACHE_DIR} -I ${BEAVER_INCLUDE_DIR} -I ${MLIR_INCLUDE_DIR}
	cd native/mlir-zig-proj && zig build --prefix ${NATIVE_INSTALL_DIR} -freference-trace --cache-dir ${MIX_APP_PATH}/zig_cache --search-prefix ${NATIVE_INSTALL_DIR} --search-prefix ../mlir-c --search-prefix ${LLVM_LIB_DIR}/.. --search-prefix ${ERTS_INCLUDE_DIR}/.. -DKINDA_LIB_NAME=beaver-v0.3.2-nif-2.16-aarch64-apple-darwin
cmake_build:
