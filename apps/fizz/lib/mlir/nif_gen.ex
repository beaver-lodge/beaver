...
defmodule Fizz.MLIR.NIFGen do
  def gen(wrapper) do
    llvm_include = Fizz.LLVM.Config.include_dir()
    {out, 0} = System.cmd("zig", ["translate-c", wrapper, "-I", llvm_include])
    ...
    {out, 0} = System.cmd("zig", ["run", dst, "-I", llvm_include], stderr_to_stdout: true)
    ...
    # |> Enum.filter(fn f -> f.name in ~w{mlirContextCreate mlirContextDestroy} end)
