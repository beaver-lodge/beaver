defmodule WasmTermABITest do
  ...
  test "declares term signatures" do
    ...
    assert TermABI.entry("ex.term.list_insert_at").params == [:i64, :i64, :i64]
    assert TermABI.entry("ex.term.list_insert_at").result == :i64
    ...
    assert TermABI.entry("ex.term.enumerable_into_map").params == [:i64, :i64]
    assert TermABI.entry("ex.term.enumerable_intersperse").params == [:i64, :i64]
    assert TermABI.entry("ex.term.enumerable_map_term_fun").params == [:i64, :i64]

    assert TermABI.entry("ex.term.enumerable_map_term_fun_c").params ==
  ...
  test "declares the pointer and noreturn special cases" do
    ...
    assert TermABI.entry("ex.term.make_fun_with_signature").params ==
             [:i64, :i64, :i64, :i64, :i64, :i64, :i64, :i64]

    assert TermABI.entry("ex.term.fun_arity").params == [:i64]
    assert TermABI.entry("ex.term.fun_result_mode").params == [:i64]
  end
